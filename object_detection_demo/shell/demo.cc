...
CONFIG load_config(const std::string &path) {
  ...
  if (values.count("score_threshold")) {
    config.score_threshold = atof(values["score_threshold"].c_str());
    if (config.score_threshold < 0.f) {
      printf("The key 'score_threshold' should >= 0.f, but receive %f!\n",
