...
def load_config(path):
    ...
    if 'score_threshold' in values:
        config['score_threshold'] = float(values['score_threshold'])
        assert config[
            'score_threshold'] >= 0, "The key 'score_threshold' should >= 0.f, but receive %f!" % config[
                'score_threshold']
        print("score_threshold: %f" % config['score_threshold'])
    # label_list
