...
if [ "$FILE_TRANSFER_COMMAND" == "lftp" ]; then
  ...
  lftp -e "cd $WORK_SPACE; mput ../../libs/FastTokenizer/$TARGET_OS/$TARGET_ABI/lib/libcore_tokenizers.so; bye" -u $SSH_DEVICE_USR_ID,$SSH_DEVICE_USR_PWD $SSH_DEVICE_IP_ADDR
  ...
  sshpass -p $SSH_DEVICE_USR_PWD scp -v -r -o ConnectTimeout=60 -o StrictHostKeyChecking=no -P $SSH_DEVICE_SSH_PORT ../../libs/FastTokenizer/$TARGET_OS/$TARGET_ABI/lib/libcore_tokenizers.so $SSH_DEVICE_USR_ID@$SSH_DEVICE_IP_ADDR:$WORK_SPACE
