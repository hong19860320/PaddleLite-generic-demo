...
MODEL_NAME=ernie_3.0_tiny_fp32
if [ -n "$1" ]; then
  MODEL_NAME=$1
fi
...
EXPORT_ENVIRONMENT_VARIABLES="export GLOG_v=5; export SUBGRAPH_ONLINE_MODE=true;export QUANT_AUTO_COMPLETE_SCALE_LEVEL=1;export QUANT_AUTO_COMPLETE_SCALE_CONFIG_FILE=./$MODEL_NAME/${NNADAPTER_DEVICE_NAMES_TEXT}_auto_complete_scale_config_file.txt;"
...
adb $ADB_DEVICE_NAME push ../../libs/FastTokenizer/$TARGET_OS/$TARGET_ABI/lib/libcore_tokenizers.so $WORK_SPACE
