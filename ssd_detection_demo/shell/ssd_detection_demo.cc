...
std::vector<RESULT> postprocess(const float *output_data, int64_t output_size, int input_width, int input_height, const std::vector<std::string> &word_labels) {
  ...
  for (int64_t i = 0; i < output_size; i += 6) {
    ...
    if (score < SCORE_THRESHOLD)
      continue;
    RESULT object;
    object.class_name = class_id >= 0 && class_id < word_labels.size() ? word_labels[class_id] : "Unknow";
    object.score = score;
...
void process(const float *input_image, std::vector<float> &output_result, const std::vector<std::string> &word_labels, std::shared_ptr<paddle::lite_api::PaddlePredictor> &predictor) {
  ...
  preprocess(input_image, INPUT_MEAN, INPUT_STD, input_width, input_height, input_data);
  ...
  std::vector<RESULT> results = postprocess(output_data, output_size, input_width, input_height, word_labels);
  ...
  for (int i = 0; i < results.size(); i++) {
    printf("[%d] %s - %f %f,%f,%f,%f\n", i, results[i].class_name.c_str(), results[i].score, results[i].x0, results[i].y0, results[i].x1, results[i].y1);
  }
...
int main(int argc, char **argv) {
  if (argc < 11) {
    printf(
        "Usage: \n"
        "./ssd_detection_demo model_path mode_type label_path image_path result_path nnadapter_device_names nnadapter_context_properties nnadapter_model_cache_dir nnadapter_model_cache_token nnadapter_subgraph_partition_config_path");
    return -1;
  }
  ...
  try {
    ...
    std::ofstream result_file(result_path, std::ios::out | std::ios::binary); // dump the output tensor to file
    result_file.write(reinterpret_cast<char *>(result_data.data()), result_data.size() * sizeof(float));
    result_file.close();
  } catch (std::exception e) {
