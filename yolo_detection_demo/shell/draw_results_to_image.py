...
if __name__ == "__main__":
    ...
    for i in range(num) :
        ...
        if score > score_threshold:
            ...
            x0 = max(int(x0 / 608.0 * src_image.shape[1]), 0)
            y0 = max(int(y0 / 608.0 * src_image.shape[0]), 0)
            x1 = min(int(x1 / 608.0 * src_image.shape[1]), src_image.shape[1])
            y1 = min(int(y1 / 608.0 * src_image.shape[0]), src_image.shape[0])
