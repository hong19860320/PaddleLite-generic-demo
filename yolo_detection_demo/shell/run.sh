...
if [ -n "$SUBGRAPH_PARTITION_CONFIG_FILE" ]; then
    NNADAPTER_SUBGRAPH_PARTITION_CONFIG_PATH="../assets/models/$MODEL_NAME/${NNADAPTER_DEVICE_NAMES}_${SUBGRAPH_PARTITION_CONFIG_FILE}"
fi
...
set -e
./$BUILD_DIR/yolo_detection_demo ../assets/models/$MODEL_NAME $MODEL_TYPE ../assets/labels/$LABEL_NAME ../assets/images/$IMAGE_NAME ../assets/results/$RESULT_NAME $NNADAPTER_DEVICE_NAMES $NNADAPTER_CONTEXT_PROPERTIES $NNADAPTER_MODEL_CACHE_DIR $NNADAPTER_MODEL_CACHE_TOKEN $NNADAPTER_SUBGRAPH_PARTITION_CONFIG_PATH
