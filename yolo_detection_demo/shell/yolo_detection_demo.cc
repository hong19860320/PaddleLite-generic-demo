...
void process(const float *image_data, std::vector<float> &output_result, const std::vector<std::string> &word_labels, std::shared_ptr<paddle::lite_api::PaddlePredictor> &predictor) {
  ...
  preprocess(image_data, INPUT_MEAN, INPUT_STD, input_width, input_height, input_data);
...
int main(int argc, char **argv) {
  if (argc < 11) {
    printf(
        "Usage: \n"
        "./yolo_detection_demo model_path mode_type label_path image_path result_path nnadapter_device_names nnadapter_context_properties nnadapter_model_cache_dir nnadapter_model_cache_token nnadapter_subgraph_partition_config_path");
    return -1;
  }
