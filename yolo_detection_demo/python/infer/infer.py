...
add_arg('model_dir', str, '../../assets/models/yolov3_mobilenet_v1_270e_coco_fp32_608', 'The directory of target paddle inference model')
add_arg('model_type', str, '1', '0: non-combined 1: combined')
add_arg('label_path', str, '../../assets/labels/coco-labels-2014_2017.txt', 'The path of label file')
...
def main(argv=None):
    ...
    scale_factor = np.array([h / float(origin_image.shape[0]), w / float(origin_image.shape[1])])
    ...
    image_shape_tensor = fluid.core.LoDTensor()
    image_shape_tensor.set(image_shape.reshape(1, 2).astype('float32'), place)
    scale_factor_tensor = fluid.core.LoDTensor()
    scale_factor_tensor.set(scale_factor.reshape(1, 2).astype('float32'), place)
    output_tensors = exe.run(program=program, feed={"im_shape": image_shape_tensor, "image": image_tensor, "scale_factor": scale_factor_tensor}, fetch_list=fetch_list, return_numpy=False)
    # Postprocess
